        ...
        if (type != chartsTypeActive) {
            ...
            $.post("<?= URL ?>ajax/charts", {name: name, start: start, end: end, type: type})
                .done(function (data) {
                    dialogBox.html(data);
                    dialogBox.dialog({
                        height: 600, width: 840, show: {effect: "fade", duration: 600},
                        buttons: {
                            "Fermer": function () {
                                $(this).dialog("close");
                            }
            ...
            if (id != projectActive) {
                ...
                window.setTimeout(function () {
                    $.post("<?= URL ?>ajax/reporting",
                        {
                            action: "getAvailableReportings",
                            id: id,
                            periodicite: reportingData.periodicite,
                            dateD: reportingData.dateStart,
                            dateF: reportingData.dateEnd,
                            par: reportingData.par
                        }
                    ).done(function (data) {
                        try {
